...
public class ImageUtils {
     ...
     * @return 是否合并成功
     */
    public static void combineImages(List<String> imgSrcList, List<String[]> topLeftPointList, int countOfLine, int cutWidth, int cutHeight, String savePath, String subfix) throws IllegalArgumentException{
        if (imgSrcList == null || savePath == null || savePath.trim().length() == 0 || countOfLine == 0) 
        	throw new IllegalArgumentException("合并图片过程:参数不能为空");
        ...
        try {
            ...
            ImageIO.write(lastImage, subfix, file);
        } catch (Exception ex) {
        	throw new IllegalArgumentException("合并图片过程:"+ex.getMessage());
        }
    }

    public static int findXDiffRectangeOfTwoImage(String imgSrc1, String imgSrc2) throws IllegalArgumentException{
        try {
        	...
        	throw new IllegalArgumentException("匹配图片过程:"+ex.getMessage());
        }
    }
